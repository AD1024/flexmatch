...
for i in {1..5}; do
    for relay_file in ${models[@]}; do
        ...
        timeout 300 ./target/debug/flexmatch ../tests/models/$relay_file rewrite.json data.json simpl.json im2col.json --maxsat
        echo ">>> Extract with ILP-ACyc"
        timeout 300 ./target/debug/flexmatch ../tests/models/$relay_file rewrite.json data.json simpl.json im2col.json --new-ilp
        echo ">>> Extract with ILP-Topo"
        timeout 300 ./target/debug/flexmatch ../tests/models/$relay_file rewrite.json data.json simpl.json im2col.json --topo-sort-ilp
    done
done
