...
def main(relay_file, output_filename, configs, use_ilp):
    ...
    model_rewrite_file = os.path.join(cur_dir, f'{output_filename}-rewritten.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--relay-file', dest='relay_file', type=str, help='Relay source file', required=True)
    parser.add_argument('--output-file', dest='output_file', type=str, help='Output file name of extracted model and analysis data (they share the same name)', required=True)
    parser.add_argument('--configs', dest='configs', nargs='+', help='Equality Saturation Configs', required=True)
